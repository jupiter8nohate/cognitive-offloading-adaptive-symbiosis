...
func main() {
	...
	if err := os.MkdirAll(filepath.Join(*root, "agent_runtime"), 0o755); err != nil {
		log.Fatal(err)
	}
...
'), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(*root, "agent_runtime", "generated_privacy_agents.go"), []byte(runtimeGo), 0o644); err != nil {
		log.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(*root, "agent_runtime", "PRIVACY_SWARM.md"), []byte(report.Markdown()), 0o644); err != nil {
		log.Fatal(err)
	}
