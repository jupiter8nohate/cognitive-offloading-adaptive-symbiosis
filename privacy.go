...
func SealDraft(plaintext, key []byte) (SealedDraft, error) {
	if len(plaintext) == 0 {
		return SealedDraft{}, errors.New("plaintext is required")
	}
	if len(key) != 32 {
		return SealedDraft{}, errors.New("key must be exactly 32 bytes")
	}
	block, err := aes.NewCipher(key)
	if err != nil {
		return SealedDraft{}, err
	}
...
func OpenDraft(sealed SealedDraft, key []byte) ([]byte, error) {
	if sealed.Version != "coas-private-draft.v1" {
		return nil, fmt.Errorf("unsupported sealed draft version: %s", sealed.Version)
	}
	if len(key) != 32 {
